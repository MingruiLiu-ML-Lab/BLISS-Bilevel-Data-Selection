...
def chunked_cross_entropy(
    logits: Union[torch.Tensor, List[torch.Tensor]], targets: torch.Tensor, logits_pre: Union[torch.Tensor, List[torch.Tensor]]=None, weight: torch.Tensor=None, chunk_size: int = 128
) -> torch.Tensor:
    ...
    if isinstance(logits, list):
        ...
        if chunk_size == 0:
            ...
            return torch.nn.functional.cross_entropy(logits, targets, weight=weight, ignore_index=-1)
    ...
    if weight is not None:
        ...
        logits = logits.reshape(-1, logits.size(-1))
        targets = targets.reshape(-1)
        loss = torch.nn.functional.cross_entropy(logits, targets, ignore_index=-1, reduction="none")
        loss = loss.view(batch_size, -1)
        weight = weight.view(-1, 1)
        unweighted_loss = None
        if logits_pre is not None:
            logits_pre = logits_pre.reshape(-1, logits.size(-1))
            loss_pre = torch.nn.functional.cross_entropy(logits_pre, targets, ignore_index=-1, reduction="none")
            loss_pre = loss_pre.view(batch_size, -1)
            unweighted_loss = loss - loss_pre.detach()
            weighted_loss = unweighted_loss.pow(2) * weight
        else:
            weighted_loss = loss * weight
        # print(f'loss: {loss.size()}')
        # print(f'weight: {weight.size()}')
        final_loss = weighted_loss.mean(dim=-1).sum()
        if unweighted_loss is None:
            return final_loss, loss.mean()
        else:
            return final_loss, unweighted_loss.mean()

    logits = logits.reshape(-1, logits.size(-1))
    targets = targets.reshape(-1)
    if chunk_size == 0:
        return torch.nn.functional.cross_entropy(logits, targets, ignore_index=-1), torch.nn.functional.cross_entropy(logits, targets, ignore_index=-1)
    ...
    loss_chunks = [
        torch.nn.functional.cross_entropy(logit_chunk, target_chunk, weight=weight, ignore_index=-1, reduction="none")
        for logit_chunk, target_chunk in zip(logit_chunks, target_chunks)
    ]
    non_masked_elems = (targets != -1).sum()
    return torch.cat(loss_chunks).sum() / max(1, non_masked_elems)
