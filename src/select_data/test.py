...
def get_distributed_hypergrad(args, fabric, score_local, grad_fx_local, grad_gy_local, z):
    ...
    gyz = [sum([(z_ * g_y).sum() for (z_, g_y) in zip(z, grad_gy_local[i])]) for i in
           range(args.micro_batch_size)]  # shape: [micro_batch_size]
    ...
    for j in range(len(weighted_fx_local)):
        ...
        result_2.append(result_1[
                            j] + weighted_fx_sum * weighted_gy_local)  # result_1 + sum_i P_i * sum_j P_j * grad_fx_j * grad_gyz_i
...
def setup(args) -> None:
    ...
    fabric.launch(
        main,
        resume=(
            Path(f"out/c4/pythia-31m/random/iter-040000-ckpt.pth")
        # out/c4/pythia-31m/bilevel-selected-data/one-model/iter-020000-ckpt.pth   out/c4/pythia-31m/random/iter-040000-ckpt.pth
        ),
...
def main(
    ...
    if args.round > 1:
        ...
        score_model_checkpoint_path = f"out/bilevel_predict_model/{model_name}/score_model_iter-{iter_num:06d}-ckpt-{model_ckpt:06d}.pth"
...
def train(args,
    ...
    #     meta_model = GPT(proxy_model.config, proxy=True)
        ...
        for epoch in range(args.epochs):
            ...
            for input_ids in train_dataloader:
                ...
                for _ in range(args.inner_steps):
                    inner_input_ids = next(iter(train_dataloader))
                    ...
                    if args.bilevel:
                        ...
                        local_score = torch.exp(
                            score_output - score_max_val) if score_max_val is not None else torch.exp(score_output)
                        ...
                        inner_sample_weights = torch.softmax(
                            torch.ones(args.micro_batch_size).cuda(inner_logits_proxy.device), dim=0)
                    ...
                    kl_div = args.gamma * kl_div_token_level(inner_logits_proxy, inner_logits_pre.detach())
                    inner_weighted_loss_tr += param_norm + kl_div  # + args.gamma * entropy(inner_sample_weights)   # + args.lamb * (inner_sample_weights.sum() - 1).pow(2)
                ...
                if args.bilevel:
                    ...
                    # local_score = torch.exp(score_output)
                    local_score = torch.exp(score_output - score_max_val) if score_max_val is not None else torch.exp(
                        score_output)
                ...
                param_norm = args.reg * sum([y.norm(2).pow(2) for y in proxy_model.parameters()])
                kl_div = args.gamma * kl_div_token_level(logits_proxy, logits_pre.detach())
                weighted_loss_tr += param_norm + kl_div  # + args.gamma * entropy(sample_weights) # + args.lamb * (sample_weights.sum() - 1).pow(2)
                G_y = torch.autograd.grad(weighted_loss_tr, proxy_model.parameters(), retain_graph=True,
                                          create_graph=True)
                ...
                hypergrad, scores_global, score_sum, softmax_score, score_max_val = get_distributed_hypergrad(args,
                                                                                                              fabric,
                                                                                                              score_local,
                                                                                                              fx_per_sample_grads,
                                                                                                              gy_per_sample_grads,
                ...
                z = [1 / param.numel() * torch.ones(param.size()).to(fabric.device) for param in
                     proxy_model.parameters()]
                    ...
                    wandb.log(
                        {
                            ...
                            "lr": lower_level_scheduler.get_last_lr()[0],
                            "z_norm": z_norm,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="pythia-31m-2048", help="Name of the model")
    ...
    parser.add_argument("--data_path", type=str, default="data/c4/pythia-410m/random/0",
    ...
    parser.add_argument("--learning_rate_influence", type=float, default=1e-5,
                        help="Learning rate for influence score model")
    ...
    parser.add_argument("--warmup_steps", type=int, default=1.0, help="warm up steps")
    parser.add_argument("--time", type=str, default="2025-01-15_05-00-00", help="Saving model checkpoint time")

    args = parser.parse_args()
    args.warmup_steps = args.max_steps * args.epochs * 0.0
    if args.time == None:
